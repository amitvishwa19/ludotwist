...
public class FacebookManager : MonoBehaviour
{
    ...
    public string playfabAuthTicket;
...
//if (UNITY_ANDROID || UNITY_IOS) && !UNITY_EDITOR
    ...
    #region Facebook SDK Initialisation
    private void InitCallback()
    {
        if (AccessToken.CurrentAccessToken != null) {
            playfabAuthTicket = AccessToken.CurrentAccessToken.TokenString;
            Debug.Log("Playfab FB access token from FB : " + playfabAuthTicket);
        }
        if (FB.IsInitialized)
        {
    ...
    {
        Debug.Log("Shaow Login Duialouge");
    ...
    public void IDRegisterButtonPressed()
    {
        loginCanvas.SetActive(false);
        idRegisterDialog.SetActive(true);
    }
    ...
    public void FacebookLoginClick() {
        if (!LoggedIn) {
            Debug.Log("Not LoggedIn, Facebook Login Clicked");
            var perms = new List<string>() { "public_profile", "email", "user_friends" };
            FB.LogInWithReadPermissions(perms, AuthCallback);
        }
    }
    ...
    private void AuthCallback(ILoginResult result)
    {
        ...
        if (FB.IsLoggedIn)
        {
            //Debug.Log("Facebook Access token : " + result.AccessToken.TokenString);
            var aToken = Facebook.Unity.AccessToken.CurrentAccessToken;
            //Debug.Log("Facebook aToken : " + Facebook.Unity.AccessToken.CurrentAccessToken);
